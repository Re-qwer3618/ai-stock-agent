...
def strategy_ui():
    ...
    # 2. [NEW] 선발대(Scout) 설정 - 유효 기간 기능 추가
    with st.expander("🚀 선발대(Scout) 진입 설정", expanded=True):
        use_scout = st.checkbox("선발대 확인 후 진입 (저점 후 반등)", value=True, help="저점 신호 발생 후, 설정한 기간 내에 상승세(선발대)가 나오면 매수합니다.")
        
        if use_scout:
            scout_pct = st.slider("선발대 기준 등락률 (%)", 1.0, 10.0, 3.0, help="당일 몇 % 이상 상승해야 선발대로 인정할까요?")
            
            # [수정된 부분] 기간 설정 슬라이더 추가
            scout_wait_days = st.slider("저점 후 대기 유효기간 (일)", 1, 20, 5, help="저점 신호가 뜬 뒤, 며칠 내에 선발대가 나오면 진입할까요? (예: 5일 설정 시 저점 후 5일 동안 반등 기회를 엿봅니다)")
            
            st.caption("👇 시가(Open) 위치에 따른 진입 허용")
            c_gap1, c_gap2 = st.columns(2)
            enable_gap_up = c_gap1.checkbox("갭상승 시 진입", value=True)
            enable_gap_down = c_gap2.checkbox("갭하락/보합 진입", value=True)
        else:
            # 변수 초기화
            scout_pct = 0.0
            scout_wait_days = 1
    ...
    return {
        ...
        "use_scout": use_scout,
        "scout_pct": scout_pct,
        "scout_wait_days": scout_wait_days, # [추가됨]
...
def logic_scout_entry(df, low_point_signal, config):
    """
    [전략 2] 선발대(Scout) 확인 로직 (유효기간 적용)
    과거 N일 이내(scout_wait_days)에 저점 신호가 발생했고,
    오늘 주가가 선발대 기준(scout_pct)만큼 올랐는가?
    """
    # 설정된 유효 기간 (기본값 1일)
    wait_days = config.get('scout_wait_days', 1)

    # 1. 최근 N일 이내에 저점 신호가 있었는가?
    # shift(1)은 '오늘 제외 과거'를 의미함.
    # rolling(window=wait_days).max()는 해당 기간 내에 True가 하나라도 있으면 True 반환
    
    was_low_recently = low_point_signal.shift(1).rolling(window=wait_days, min_periods=1).max()
    was_low_recently = was_low_recently.fillna(0).astype(bool)
    ...
    # 최종 진입 신호: (최근에 저점 찍음) AND (오늘 급등) AND (갭 조건 만족)
    final_signal = was_low_recently & is_scout_candle & gap_allowed
    
    # 메시지 생성 (Wait 기간 정보 포함)
    msg = np.where(final_signal, 
                   f"Scout(Within {wait_days}days, Chg>{config.get('scout_pct')}%)", 
                   "")
